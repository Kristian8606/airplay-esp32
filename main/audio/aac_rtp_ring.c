...
void aac_rtp_ring_invalidate_range(aac_rtp_ring_t *r, uint32_t from_rtp,
                                   uint32_t until_rtp, uint32_t generation) {
  if (!r || (int32_t)(until_rtp - from_rtp) <= 0) {
    return;
  }

  for (uint32_t i = 0; i < AAC_RTP_SLOT_COUNT; ++i) {
    aac_slot_tag_t *tag = &r->tags[i];
    uint32_t state = __atomic_load_n(&tag->state, __ATOMIC_ACQUIRE);
    if (state != AAC_SLOT_READY || tag->generation != generation) {
      continue;
    }
    if ((int32_t)(tag->rtp - from_rtp) >= 0 &&
        (int32_t)(until_rtp - tag->rtp) > 0) {
      uint32_t expected = AAC_SLOT_READY;
      if (__atomic_compare_exchange_n(&tag->state, &expected, AAC_SLOT_FREE,
                                      false, __ATOMIC_ACQ_REL,
                                      __ATOMIC_ACQUIRE)) {
        __atomic_sub_fetch(&r->ready_slots, 1U, __ATOMIC_RELAXED);
      }
